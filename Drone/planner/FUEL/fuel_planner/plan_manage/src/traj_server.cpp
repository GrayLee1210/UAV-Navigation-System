...
int main(int argc, char** argv) {
  ...
  // DISABLED for real flight: this publishes absolute position cmds at
  // (init_x,init_y,init_z) which would snap the real drone there. Gated behind
  // param traj_server/sim_takeoff (default false). Matches ego traj_server:
  // nothing is published until the first trajectory is received.
  bool sim_takeoff = false;
  nh.param("traj_server/sim_takeoff", sim_takeoff, false);
  if (sim_takeoff) {
    for (int i = 0; i < 100; ++i) {
      cmd.position.z += 0.01;
      pos_cmd_pub.publish(cmd);
